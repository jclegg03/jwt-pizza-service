...
response=$(curl -k -s -X PUT $host/api/auth -H 'Content-Type: application/json' -d "{\"email\":\"$ADMIN_EMAIL\", \"password\":\"$ADMIN_PASSWORD\"}")
token=$(echo $response | jq -r '.token')

# Add users
curl -k -X POST $host/api/auth -d "{\"name\":\"pizza diner\", \"email\":\"$DINER_EMAIL\", \"password\":\"$DINER_PASSWORD\"}" -H 'Content-Type: application/json'
curl -k -X POST $host/api/auth -d "{\"name\":\"pizza franchisee\", \"email\":\"$FRANCHISE_EMAIL\", \"password\":\"$FRANCHISE_PASSWORD\"}" -H 'Content-Type: application/json'
...
curl -k -X POST $host/api/franchise -H 'Content-Type: application/json' -d "{\"name\": \"pizzaPocket\", \"admins\": [{\"email\": \"$FRANCHISE_EMAIL\"}]}" -H "Authorization: Bearer $token"
